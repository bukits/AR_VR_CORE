...
void RubikCube::MapColors(const char* colors) {
    if (dim_ == 3 || dim_ == 5) {
        ...
        while (color_map_idx < 6) {
            ...
            for (const auto & corner_coord : corner_coords) {
                int corner_mask = 0;
                char new_color_char = '\0';

                for (int i = 0; i < 6; i += 2) {
                    char color_char = colors[corner_coord[i] * piece_num_ + corner_coord[i + 1]];
...
void RubikCube::RotateCube(const ROTATE_CUBE_DIR& dir) {
    ...
    char* tmp_face = new char[piece_num_];
...
std::string RubikCube::GetCubeString(const bool& is_color/* = false*/) {
    char* tmp_faces = new char[std::strlen(faces_)];
